...
test("it should render the correct default values", async () => {
  const props = {
    isOpen: true,
    denyClick: () => null,
    confirmClick: () => null,
  ...
  expect(screen.getByRole("button", { name: /confirm button/i })).toHaveTextContent(
    "Yes"
  );
...
test("it should trigger the button actions on click", async () => {
  const props = {
    isOpen: true,
    onClose: jest.fn(),
    denyClick: jest.fn(),
